...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'clear-both clearfix marginbottom-xlarge' ); ?>>
	<header class="entry-header position-relative">
		...
		<?php
		if ( is_home() ) {
			echo '<h2 class="margintop-none marginbottom-xsmall entry-title f2 b sans-serif">';
		} else {
			echo '<h1 class="margintop-none marginbottom-xsmall entry-title f2 b sans-serif">';
		} ?>
			<?php
			if ( is_sticky() ) {
		...
		<div class="posted-on i small lightestgreytext margintop-small marginbottom-medium">
			<?php
				...
				*/
				...
				printf( '<span class="screen-reader-text">%1$s </span>%2$s by %3$s in',
